...
async function getReferralCodes(): Promise<ReferralCodesProps[]> {
    ...
    while (needToFetchMoreCodes) {
        ...
        const referralCodes = (referralCodesResponse?.data?.referralCodes || []).map(code => ({
            id: code.id,
            owner: code.referrer.id,
...
export async function getReferrerRewards(referralCodes: ReferralAndOwner[], weeksToGoBack: number) {
...
//             const feesPaid = sum((referee.dayData || []).map(d => Number(formatUnits(d.fee, 18))))
            ...
            for (const dayData of dayDatas) {
                ...
                for (const data of traderData) {
                    if (!feesPerTrader[data.trader.id]) {
                        ...
                        feesPerTrader[data.trader.id] = feesPerTrader[data.trader.id] + Number(data.fees)
                    }
                }
            }
...
async function getRefereeRewards(referralCodes: ReferralAndOwner[], weeksToGoBack: number) {
    ...
    for (const feeData of codeFees) {
        ...
        for (const referee of Object.keys(feesPaidByTraders)) {
            ...
            if (!aggregateFeesPaidByReferees[referee]) {
                ...
                aggregateFeesPaidByReferees[referee] = aggregateFeesPaidByReferees[referee] + feesPaid
            }
        }
    }
    ...
    const rewards = await Promise.all(
        Object.keys(aggregateFeesPaidByReferees).map(async referee => {
            ...
            const rebate = calculateRefereeRewards(Number(aggregateFeesPaidByReferees[referee]), stakedPerp)
            return {
                referee,
...
export default function Report() {
    ...
    const { data: referrerRewardsCSV, isSuccess: generatedReferrerRewardsCSV } = useQuery(
        ["referrerRewards"],
        () => getReferrerRewards(referralCodes, 2),
        {
            enabled: referralCodes?.length > 0,
        },
    )

    const { data: refereeRewardsCSV, isSuccess: generatedRefereeRewardsCSV } = useQuery(
        ["refereeRewards"],
        () => getRefereeRewards(referralCodes, 2),
        {
            enabled: referralCodes?.length > 0,
        },
    )
