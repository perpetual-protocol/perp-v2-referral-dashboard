...
function PartnerRow({ eligibility }: PartnerRowProps) {
    ...
    const upgrade = async () => {
        ...
        try {
            showToast("Upgrading...")
            await assignVip(library.getSigner(), eligibility.address, String(Number(currentTier) + 1))
    ...
    return (
        <tr key={`${eligibility.address}`} className="border-t border-white border-opacity-10 font-normal">
            <td className="w-1 py-4 text-left">Tier {eligibility.tier}</td>
            <td className="w-1 py-4 text-left">{eligibility.code}</td>
            <td className="w-1 py-4 text-left">{eligibility.isInTrialPeriod ? "Yes" : "No"}</td>
            <td className="w-1 py-4 text-left">{formatDistanceToNow(fromUnixTime(Number(eligibility.vipSince)))}</td>
...
export default function Admin(props: Props) {
    ...
    const batchUpdate = async () => {
        ...
        const partnersNotInTrial = eligiblePartners.filter(partner => !partner.isInTrialPeriod)
        ...
        try {
            ...
            await batchUpdatePartners(library.getSigner(), partnersToUpdate, tiersToUpdateTo)
            await refreshEligibility()
            showToast("Upgraded")
        } catch (error) {
            console.log(error)
                ...
                <div className="w-full h-full flex justify-center p-16">
                    <div className="flex flex-col">
                        ...
                        <div className="mt-8 w-full flex justify-center flex-col items-center">
                            <h3 className="text-white text-lg font-semibold">VIP Partners List</h3>
                            ...
                            >
                                <thead className="uppercase font-medium text-xs text-perp-gray-50">
                                    <tr>
                                        ...
                                        <th className="w-1 pb-4 pt-1 text-left">Current Eligibility</th>
                                ...
                                <tbody>
                                    {!isLoadingEligiblePartners &&
                                        ...
                                        (eligiblePartners || []).map(eligibility => (
                                            <PartnerRow key={eligibility.address} eligibility={eligibility} />
                                        ))}
                                </tbody>
                            </table>
                        </div>
