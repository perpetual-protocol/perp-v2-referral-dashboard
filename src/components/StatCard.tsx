...
export default function StatCard(props: Props) {
    const { title, icon, change, value, format, max, isLoading, subtext, tooltip, state = "normal", span = 4 } = props
    if (isLoading) {
        return (
            <Skeleton
                height={120}
    ...
    const changeColor = change && change > 0 ? "text-perp-light-green" : "text-perp-red"
        ...
        <div className={`rounded-2xl bg-perp-gray-300 py-5 px-6 col-span-12 sm:col-span-6 md:col-span-${span}`}>
            ...
            <div className="pb-2">
                <div className="flex items-center">
                    ...
                    {change !== undefined && (
                        <span className={`${changeColor}`}>{numeral(change).format("+0.0%")}</span>
                    )}
                </div>
            </div>
