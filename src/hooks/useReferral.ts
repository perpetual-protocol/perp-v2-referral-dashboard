...
import { last, nth, sumBy } from "lodash"
...
export async function callReferrerContract(provider: BaseProvider, method: string, params: any[]) {
    const contract = new Contract(CONTRACT_ADDRESS, PerpetualProtocolABI, provider)
    try {
...
export async function calculateRefereesWeeklyVolume(referralCode: string, weeks: number = 7) {
    ...
    const volumes = (refereesDayData || []).map(day =>
        sumBy(day.data.referralCodeDayDatas, (dayData: any) => Number(dayData.tradingVolume)),
    )
...
async function getWeeklyNewReferees(referralCode: string, customWeeks: number = 7) {
...
export default function useReferral() {
    ...
    const { data: weeklyReferralCodeVolume, isLoading: isLoadingWeeklyVolume } = useQuery(
    ...
    const { data: referralCodeDayDatasResponses, isLoading: isLoadingDayDatas } = useQuery(
    ...
    const currentWeeklyReferralVolume = Number(last(weeklyReferralCodeVolume)?.volume)
    const lastWeekReferralVolume = Number(nth(weeklyReferralCodeVolume, -2)?.volume)

    const weeklyReferralVolumeChange = getVolumeChange(currentWeeklyReferralVolume, lastWeekReferralVolume)

    const referralCodeDayDatas = referralCodeDayDatasResponses?.map(r => r.data?.referralCodeDayDatas) || []
    ...
    const totalReferees = Number(referrerResponse?.data?.trader?.referrerCode?.numReferees) - 1
    const referees = referrerResponse?.data?.trader?.referrerCode?.referees
    const referralLink = `${PERP_DOMAIN}?code=${referralCode}`
    const isVIP = Number(referrerResponse?.data?.trader?.referrerCode?.vipTier) >= 3
    const _normalTier = Object.values(referrerTiers)
        .filter(t => t.minFees === 0)
        .reverse()
        .find(t => sPerp >= t.staked)
    const _vipTier = Number(referrerResponse?.data?.trader?.referrerCode?.vipTier)
    const vipTier = isVIP ? _vipTier : _normalTier?.tier || 1
    const vipSince = Number(referrerResponse?.data?.trader?.referrerCode?.vipSince)
