...
export function getTraderDayData(account: string, customWeeks: { start: number; end: number }[]) {
    const timestamps = customWeeks || getLastNWeeks(7)
    const promises = timestamps.map(async timestamp => {
        const res = await SUBGRAPH(`
        query {
          traderDayDatas(where: { trader: "${account.toLowerCase()}", date_gte: ${timestamp.start}, date_lt: ${
            timestamp.end
        }}, orderDirection: desc, orderBy: date) {
                id
                tradingVolume
                date
                fee
...
export default function useTrading(customTimestamps?: { start: number; end: number }[]) {
    ...
    const dayDatas = (data || []).map(d => d.data?.traderDayDatas)
